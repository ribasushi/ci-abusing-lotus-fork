	...
	if err := b.db.View(func(txn *badger.Txn) error {
		for i := range kvs {
			...
			if err != nil {
				...
				// Got to insert that, check it is supported
				_, err := isMultihashSupported(blocks[i].Cid().Hash())
				if err != nil {
...
func (b *Blockstore) ForEachKey(f func(cid.Cid) error) error {
	// unfortunately this is used in splitstore, so have to reimplement...
